function Isosurfaces( volume, isovalue )
{
    ...
    for ( var z = 0; z < volume.resolution.z - 1; z++ )
    {
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
		    ...
		    var i0 = get_vertices_id(cell_index, vid0);
		    var i1 = get_vertices_id(cell_index, vid1);
		    var i2 = get_vertices_id(cell_index, vid2);
		    var i3 = get_vertices_id(cell_index, vid3);
		    var i4 = get_vertices_id(cell_index, vid4);
		    var i5 = get_vertices_id(cell_index, vid5);

                    var v01 = interpolated_vertex( v0, v1, volume.values[i0][0], volume.values[i1][0], isovalue );
                    var v23 = interpolated_vertex( v2, v3, volume.values[i2][0], volume.values[i3][0], isovalue );
                    var v45 = interpolated_vertex( v4, v5, volume.values[i4][0], volume.values[i5][0], isovalue );
    ...
    var C = Math.round(normalized(isovalue, smin, smax) * 255);
    //material.color = new THREE.Color().setHex( cmap[C][1] );
    geometry.faces.map(face => (face.color = new THREE.Color().setHex(cmap[C][1])));
    ...
    function interpolated_vertex( v0, v1, value0, value1, s )
    {
	var t =  normalized(s,value0, value1);
    ...
    function get_vertices_id(cell_index, v)
    {
	var id = cell_index - 1 ;
	var lines = volume.resolution.x;
        var slices = volume.resolution.x * volume.resolution.y;
